...
def main() -> None:
    ...
    for problem_idx, problem in enumerate(problems):
        for sample_idx in range(cfg.samples_per_problem):
            baseline_prompt = BASELINE_ACTOR_PROMPT.format(question=problem.question)
            baseline_response = actor.generate(baseline_prompt)
            baseline_correct = is_correct_prediction(baseline_response.final_answer, problem.answer)

            evasion_prompt = EVASION_ACTOR_PROMPT.format(question=problem.question)
            evasion_response = actor.generate(evasion_prompt)
            monitor_label = monitor.classify(evasion_response.reasoning)
            monitor_detected = monitor_label == "DETECTED"
            evasion_correct = is_correct_prediction(evasion_response.final_answer, problem.answer)
            ...
            records.append(
                {
                    ...
                    "baseline_reasoning": baseline_response.reasoning,
                    "baseline_final_answer": baseline_response.final_answer,
                    "baseline_correct": baseline_correct,
                    "baseline_reasoning_tokens": baseline_response.token_count,
                    "evasion_reasoning": evasion_response.reasoning,
                    "evasion_final_answer": evasion_response.final_answer,
